...
def send_payment(price, phone, email, title, order_number, return_url):
        ...
        ReceiptItem(
            {
                ...
                "quantity": 2.0,
                "amount": {"value": 250.0, "currency": Currency.RUB},
                "vat_code": 2,
            }
        ),
        {
            ...
            "quantity": 1.0,
            "amount": {"value": 100.0, "currency": Currency.RUB},
            "vat_code": 2,
        },
    ]

    builder = PaymentRequestBuilder()
    builder.set_amount({"value": price, "currency": Currency.RUB}).set_confirmation(
        {"type": ConfirmationType.REDIRECT, "return_url": return_url}
    ).set_capture(False).set_description(title).set_metadata(
...
def main():
    pass
    # send_payment('2000', '+79778120890', 'zay@ya.ru', 'bouquet', '112')
